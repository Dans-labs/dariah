...
[Webpack]({{site.webpack}}). All hand written code is monitored syntactically
and stylistically by [eslint]({{site.eslint}}).
		...
		* `dariah`
			...
			* `server`
				...
				* `models` yaml files defining the [data model](Model) (these files have
					...
					* `tables`
						* _table_`.yaml` per _table_ modeling
					...
					* `js`
						* `app`
							* `dux` connectors between React components and the Redux state.
								...
								contain the following sections: _actions_, _reducer_,
								_selectors_, _helpers_.
