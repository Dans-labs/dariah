...
It uses the data model and the permission model to serve any data to any user in such a way that no data is sent from server to client that the current user is not entitled to see.
