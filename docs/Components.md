...
statistics in the form _subtotal of total_.
...
Displays a _collective_ checkbox for a [facet filter](#byvalue) with many
facets.
...
From this object the property `pathname` will be read, which will be split into
directory, file and extension parts. The extension is used to switch to the
component for that type of documents.
...
Show [Markdown]({{site.markdownDoc}}) text, coming from files on the server. The
...
**NB:** On iOS this does not work well, only the first page of the PDF gets
...
descendant of a [redux-form]({{site.reduxFormBase}})-enabled `<form>`-carrying
...
component is rather dumb, it needs to be passed an `onClick` handler that will
...
enhanced by [redux-form]({{site.reduxFormBase}}). Because it is outside a
...
A complex component! It is a facet filter for the field _country_, using
...
The map is a [Leaflet]({{site.leaflet}}) module on a blank pane, with a
[geojson]({{site.libBase}}/europe.geo.js) file of country boundaries laid out on
...
An array of entity ids that are the allowed elements when the field is a
...
[redux-form()]({{site.reduxFormBase}}/{{site.reduxFormV}}/docs/api/ReduxForm.md/)
into [ItemForm](#itemform), the parent of this component, and they will be
passed on to _Field_ and _FieldArray_, so that they can do their magic.
...
one or more
[Field]({{site.reduxFormBase}}/{{site.reduxFormV}}/docs/api/Field.md/) or
[FieldArray]({{site.reduxFormBase}}/{{site.reduxFormV}}/docs/api/FieldArray.md/)
components (which are provided by [redux-form]({{site.reduxFormBase}}).
...
We enhance _textareas_ by offering markdown previews of their content. See
...
Presents the value(s) of a read-only field, based on _initial values_. Note that
...
Contains the attribute `onChange` by which the form value of this field can be
changed.

###### `widget` function

A function that when passed a handler, will return a React fragment. When this
fragment receives a click, the event handler will be called.

###### `setValue` any value

The value that will be passed to the handler of `widget`, when it receives a
click.

### Task

This is a form input component meant to be passed to a
[Field]({{site.reduxFormBase}}/{{site.reduxFormV}}/docs/api/Field.md/)
...
Contains attributes related to validation and edit state; is the value changed
and unsaved, invalid, and of so, for what reason?

###### `input` object

Contains attributes related to the actual value that is being held. These
attributes are passed verbatim to the underlying `<input />`.

###### `type` string

The _type_ of `<input type="..." />`. It will go to the place of the dots.
...
Validation function. Takes a value and return undefined if all is well, and
otherwise a reason why not.
...
The names of the individual fields. If the collective name of this field is
`foo`, than this array contains `foo[0]`, `foo[1]`, etc., as many as their are
values. These names are just strings.
...
injected by the wrapper
[reduxForm()]({{site.reduxFormBase}}/{{site.reduxFormV}}/docs/api/ReduxForm.md/)
...
are just passed on to _Field_ and _FieldArray_, so that they can do their magic.
...
Validation and normalization are done per individual _Field_.
...
A function that is invoked when the form is submitted. This function is passed
from [redux-form]({{site.reduxFormBase}}) and handles all the form submission
machinery. See [ItemEdit](#itemedit).
...
The component does not show _save_ and _reset_ buttons. It is meant for controls
...
This function can be used to switch this component from read-only view to edit
view and back. It will be passed on to the widget that also has the edit
controls for submitting and resetting the form.
...
Whether the form contains invalid, values in any of its fields. The form uses
...
A function that can reset the form. Resetting means: changing all edited values
back to the initialValues.
...
We use [redux-form]({{site.reduxFormBase}}) for displaying forms, filling them
out, submitting them, sending the values to the database, validating and
normalizing values.

Although _redux-form_ has an awesome functionality, it is far from trivial to
get it integrated.

The work horses are the
[Field]({{site.reduxFormBase}}/{{site.reduxFormV}}/docs/api/Field.md/) and
[FieldArray]({{site.reduxFormBase}}/{{site.reduxFormV}}/docs/api/FieldArray.md/)
components.
...
An object with the initial values of all fields that are being managed by the
form as a whole.

###### `isactive` string

A CSS class name to add extra formatting if the record in question is deemed
_inactive_. The notion of active items is defined in the duct
[workflow](Dux#workflow).
...
Since this component has to render records in a grid view, it must know
something about the widths of the columns. That information is contained in this
prop, as a CSS style per column.

###### `alt` bool

The component must know whether it is an ordinary grid row, or whether the
fields should be expanded into a vertical form.

###### `nextAlt` function

This function can be used by a control by which the user can switch between row
view and vertical view of the record.
...
Slice of the state, obtained with [getGrid](Dux#getgrid), which holds sorting
information of table grids.

###### `gridTag` string

Key under which the component finds its information about which columns are
sorted in what order and direction.

### Task

This component shows a table as a grid. It uses [CSS flex-box]({{site.flexBox}})
for the grid layout. There is also [CSS grid]({{site.grid}}) but at the time of
...
The item to navigate to, by its MongoDB id. It will be opened and scrolled into
view.
...
The list of notifications that have been issued since the beginning of the
session or since the last time that the user has cleared the messages.
...
The indexes of the last message and the last notable message, and the kind of
the last notable message, which is one of `error`, `warning`, `special`. Only
the kind `info` is non-special.

When the notifications are displayed, the panel will be scrolled to the last
notable message if there is one, otherwise to the last message.
...
list is a detail records list, there is also a control to _open_ all items in
...
[tables]({{site.appBase}}/dux/tables.jsx).
...
Whether to display a select widget where the user can make multi-selections or
...
global state. This is about whether the options have popped up and what search
text the user has entered in the filter box.
...
[react-select]({{site.reactSelect}}) component on GitHub. However, it has some
...
* plays well with [Redux-Form]({{site.reduxFormBase}});
...
Displays a string of the form _subTotal_ `of` _total_. If one of the two is
...
These are object passed by [React-Router]({{site.reactRouter}}). From this the
navigation route that the user has followed to arrive here, can be read.
...
The position of the tooltip relative to its target. Should be one of `top bottom
left right`.
...
Whether the tooltip should be triggered by focus actions only, or hovering as
well. Handy for text input elements (`<input type="text">` or `<textarea>`) if
you want to show editing help, but only if the user is actually typing in the
field.
