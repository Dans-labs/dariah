...
*   `select` **string**; sometimes a list is fetched as a whole, sometimes only
    *my own* records are displayed and yet other times only records that are the
...
*inactive*. The notion of active items is defined in the duct
...
The position of the tooltip relative to its target. Should be one of
