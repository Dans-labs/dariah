...
class Controller(object):
    ...
    def data(self, controller, Perm):
        return self._errorWrap(
            lambda: self._data(controller, Perm), controller, controller == N.mod
        )
    ...
    def ourlist(self, name):
        table = getq(N.table)
        complete = getq(N.complete) == N.true
        ourFields = DM.get(table, {}).get(N.ourFields, [])
        return self.DB.getList(name, table, titleOnly=not complete, my=ourFields)
    ...
    def _data(self, controller, Perm):
        ...
        if callable(method):
            ...
            result = self.DB.stop(
                {
                    N.msgs: [
                        {N.kind: N.error, N.text: "wrong method: {}".format(controller)}
                    ],
                }
            )
        return result
            ...
            result = self.DB.stop(
                {
                    N.data: data,
                    N.msgs: [
                        {N.kind: N.error, N.text: "server error: {}".format(message)}
                    ],
                }
            )
        return result
