# PURPOSE
# 
# (Re)import legacy data and system tables into a new or existing database.
# The legacy data comes from FileMaker and will be converted to MongoDB.
#
# NOTE
#
# We do not transfer MongoDB data from the development system to the production system.
# That would interfere too much with ongoing work on the development and the production
# databases.
#
# The ideal is to import legacy once, and then go working in production.
# the reality is that there are constantly reasons to rerun the FileMaker conversion, reorganize the
# data, and add new system tables.
# So we support a scenario where we can reimport the Filemaker legacy data.
# This script is meant to do that.
# If you want to (re)import in development data, run it on the dev server, in development mode.
# If you want to (re)import in production data, run it on the production server, in production mode.
#
# REQUIREMENTS
#
...
#
# See later, under DISCUSSION how this is achieved.
# 
# USAGE
#
...
#    Troublesome means: not pristine, but occurring in the records to be imported.
...
# 5. If the script changes, but the number and order of records that are generated remains the same
#    the generated ids are still the same.
#
# This does not guarantee that no relationshipos will break.
# The only case where things might go wrong are the non-pristine records.
...
#
# Note on the user table
#
# All production users in the system are not pristine.
# So they will be untouched.
# No legacy data refers to production users. So the legacy users are disjoint from the
