...
export const memoize = (f, levels, config) => {
  ...
  const resolveArg = (fArg, byId) => {
    ...
    if (fArg == null || (!byId && fArgType !== 'function')) {
      return jString(fArg)
    }
  ...
  const resolveToLevel = (fArg, level) => {
    if (level === -1) {
      return resolveArg(fArg, false)
    }
    if (level === 0) {
      return resolveArg(fArg, true)
    }
    const fArgType = typeof fArg
    if (fArg == null || fArgType !== 'object') {
      return resolveArg(fArg)
    }
    if (Array.isArray(fArg)) {
      return fArg.map(x => resolveToLevel(x, level - 1))
    }
    return Object.keys(fArg)
      .sort()
      .map(x => [x, resolveToLevel(fArg[x], level - 1)])
  }
  ...
  const memoF = (...fArgs) => {
    ...
    const memoKey =
      levels == null
        ...
        : jString(
            fArgs.map((fArg, i) => resolveToLevel(fArg, useLevels[i] || 0)),
          )
    ...
    if (result == null) {
      ...
      setTimeout(
        () => clearCache(memoKey),
        (useConfig.clearCache || 1800) * 1000,
      )
    } else {
      retrieved += 1
      if (process.env.NODE_ENV === `development`) {
