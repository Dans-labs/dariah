...
const putDetailFragments = (
  ...
  detailFragments.map(({ name, detailListIds }, i) => {
    const widthStyle = widthStyles[nFields + i]
    const nDetails = detailListIds.length
    return (
      <div key={name} className={'grid-cell value-col-grid'} style={widthStyle}>
        {nDetails}
      </div>
    )
  })

const fieldsButton = memoize(
  (nextAlt, table, eId, mayUpdate, on) =>
    mayUpdate ? (
      <Tooltip
