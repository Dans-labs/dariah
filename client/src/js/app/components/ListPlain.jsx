...
class ListPlain extends Component {
  ...
  gotoItem = eId => {
    ...
    putAlt(theAlt)
    const { putAlt: putAltE } = compileAlternatives(
      `edit-${table}-${eId}`,
      nAlts,
      initial,
      dispatch,
  ...
  scroll = domElem => {
    if (domElem != null) {
      ...
      // this is a non-standard function, only supported by webkit, not by firefox
  ...
  openAll() {
    const {
      props: { alter, alterSection, table, listIds, expand, dispatch },
    } = this
    if (expand) {
      handleOpenAll(
        alter,
