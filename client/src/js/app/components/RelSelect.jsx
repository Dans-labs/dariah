...
import { checkDisabled } from 'fields'
import { makeSubmitTime, makeChangeSave } from 'edit'
  ...
  <Tooltip
    tip={
      selected
        ...
        : multiple ? `add ${label}` : `select ${label}`
    }
    at={'bottom'}
  >
    <p {...attributes} onClick={selected ? null : onHit}>
      {label}
    </p>
  </Tooltip>
)
...
const changeSel = memoize(
  (selectTag, multiple, value, val, onChange, dispatch) => () => {
    if (multiple) {
      if (!value.includes(val)) {
...
const Head = ({
  ...
  return (
    <Tooltip
      ...
      at={'right'}
    >
      <span {...attributes} onClick={handle(dispatch, togglePopUp, selectTag)}>
        {label} <span className={`fa fa-arrow-${dir} ${dir}`} />
      </span>
    </Tooltip>
  )
}
      ...
      >
        {value != null && value.length ? (
          value.map(val => {
            ...
            return (
              <span key={val} {...attributes}>
                <Tooltip tip={`remove ${lab}`} at={'bottom'}>
                  <span
...
const Typing = ({ selectTag, search, dispatch, placeHolder }) => (
  <span className={'option-type'}>
    {search ? (
      <Tooltip tip={'clear typing'} at={'bottom'}>
        <span
          className={'button-tag'}
    ...
    {'\xa0'}
    <Tooltip tip={placeHolder} at={'left'}>
      <input
        className={'typing'}
        type={'text'}
...
const Options = ({
  ...
  const sizeClass = oLen >= 20 ? 'o-large' : 'o-small'

  return (
    <div className={`options ${sizeClass}`}>
      {multiple || value == null || value == emptyS ? null : (
        <Tooltip tip={'clear selection'} at={'bottom'}>
          <span
      ...
      !value.includes(search) ? (
        <Tooltip tip={`add ${search} as a NEW option`} at={'bottom'}>
          <span
            className={'new tag'}
      ...
      {options.map(({ value: val, label: lab }) => {
        if (
          ...
          const selected =
            (multiple && value.includes(val)) || (!multiple && value === val)
          const attributes = makeAttributes(
            val,
            `tag option ${selected ? 'selected' : emptyS}`,
          )
          return (
            <RelOption
              key={val}
...
const RelSelect = ({
  ...
  const { options, optionLookup } = compileOptions(
    tables,
    table,
    allowed,
    field,
  ...
  const realPopUp =
    ...
    (popUpIfEmpty &&
      (value == null ||
        ...
        (!multiple && !value)))
  const itemType = allowNew === true ? 'value' : allowNew || emptyS
  const addItem = itemType ? ` or add a new ${itemType}` : emptyS
  const placeHolder = `filter remaining options${addItem}`
  return (
    <div className={`select ${multiple ? 'multiselect' : emptyS}`}>
      {multiple ? (
        <Tags
