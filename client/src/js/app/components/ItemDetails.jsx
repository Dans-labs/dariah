    ...
    <div className={'grid fragments'}>
      {detailFragments.map(
        ({
          ...
          const expand =
            expandSpec === 'own' ? isOwner(tables, table, eId, me) : expandSpec
          return (
            <div key={name} className={'grid-row'}>
              {!expand ? (
                ...
                >
                  <span className={'link'} onClick={nextAlt}>
                    <span
                      className={`button small fa fa-angle-${
                        alt === 0 ? 'down' : 'up'
                      }`}
                    />
                    {alt === 0
                      ? `${nDetails} ${
                          nDetails === 1 ? detailThing : detailThings
                        }`
                      : emptyS}
                  </span>
                </Tooltip>
