...
const mustCorrect = {
  ...
  bottom: (container, obj, rawPos) => rawPos.top + obj.height > container.height,
  left: (container, obj, rawPos) => rawPos.left < 0,
  right: (container, obj, rawPos) => rawPos.left + obj.width > container.width,
}

const adapt = {
  top: (container, obj, corrPos) => {corrPos.top = 0},
  bottom: (container, obj, corrPos) => {corrPos.top = container.height - obj.height},
  left: (container, obj, corrPos) => {corrPos.left = 0},
  right: (container, obj, corrPos) => {corrPos.left = container.width - obj.width},
}
...
class Tooltip extends Component {
  ...
  storeTip = domElem => {this.tip = domElem}
  storeArrow = domElem => {this.arrow = domElem}
