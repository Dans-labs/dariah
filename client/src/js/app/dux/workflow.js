...
export const assessmentScore = memoize((tables, aId) => {
	...
	const myCriteriaEntries = Object.keys(ceEntities)
		...
		.map(ceId => {
			const {
				[ceId]: { values: { criteria: thisCid, score: thisSid } },
			} = ceEntities
			const {
				[thisSid]: { values: { score = 0 } = emptyO } = emptyO,
			} = sEntities
			const maxScore = max(
	...
	const relevantCriteriaEntries = myCriteriaEntries.filter(x => x.score >= 0)
	...
	const overall =
		relevantMax == 0 ? 0 : Math.round(relevantScore * 100 / relevantMax)
