...
const moreToCore = (state, table, _id, values, newState) => {
  const { core: { [table]: coreTableData = emptyO } } = state
  const { fields: coreFields, coreEntities = emptyO } = coreTableData
  const { [_id]: coreEntityData = emptyO } = coreEntities
  const { values: coreValues = emptyO } = coreEntityData
  const newCoreValues = {}
  Object.keys(coreFields).forEach(field => {
    if (JSON.stringify(coreValues[field]) != JSON.stringify(values[field])) {
      newCoreValues[field] = values[field]
    }
  })
  const newCore = Object.keys(newCoreValues).length ?
    merge(newState, { core: { [table]: { entities: { [_id]: { values: newCoreValues } } } } }) :
    newState
  return newCore
}
...
const flows = {
  ...
  /*
  //not OK: too many state updates
  fetchItem(state, { data, table }) {
    if (data == null) {return state}
    const { values, values: { _id } } = data
    const newState = merge({}, state, {
      more: { [table]: { entities: { [_id]: data } } },
    })
    return newState
    //return moreToCore(state, table, _id, values, newState)
  },
  */
  // OK: good logic, good behaviour, but a pain to program
  fetchItem(state, { data, table }) {
    if (data == null) {return state}
    const { values, values: { _id } } = data
    const newState = update(state, {
      more: {
        $apply: more => update(more || {}, {
          [table]: {
            $apply: t => update(t || {}, {
              entities: {
                $apply: e => update(e || {}, {
  ...
  modItem(state, { data, table }) {
    ...
    const newState = merge({}, state, { more: { [table]: { entities: { [_id]: data } } } })
    newState.more[table].entities[_id].values = values

    for (const { _id, rep, relTable, field } of newValues) {
      newState.more[relTable].entities[_id] = { values: { _id, rep } }
      newState.more[table].valueLists[field].unshift(_id)
    }
    return newState
  },
  insertItem(state, { data, table, select }) {
    ...
    return mergeWith({}, state, { more: { [table]: { lastInserted: _id, fields, entities: { [_id]: data }, [select]: [_id] } } }, addKey(select))
  },
...
export default makeReducer(flows, { core: emptyO, more: emptyO })
