...
export const addColumn = (gridTag, column, direction) => ({
	type: 'addColumn',
	gridTag,
	column,
	direction,
})
export const delColumn = (gridTag, column) => ({
	type: 'delColumn',
	gridTag,
	column,
})
export const turnColumn = (gridTag, column) => ({
	type: 'turnColumn',
	gridTag,
	column,
})
...
const flows = {
	...
	addColumn(state, { gridTag, column, direction }) {
		...
		return {
			...state,
			[gridTag]: (sortSpec || emptyA)
				.filter(x => x[0] !== column)
				.concat([[column, direction]]),
		}
	},
	delColumn(state, { gridTag, column }) {
		...
		return {
			...state,
			[gridTag]: (sortSpec || emptyA).filter(x => x[0] !== column),
		}
	},
...
export const compileSortedData = memoize(
	(tables, table, listIds, sortSpec, settings) => {
