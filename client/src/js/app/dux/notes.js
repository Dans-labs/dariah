...
const subFlows = {
  progress(state, { desc }) {
    return update(state, {
      messages: {
        $push: [{ kind: 'special', text: `still waiting for ${desc}` }],
      },
    })
  },
  pending(state, { desc, busy, extraMsgs }) {
    return update(state, {
      messages: {
        $push: [...extraMsgs, { kind: 'special', text: `waiting for ${desc}` }],
      },
      busy: { $set: busy + 1 },
    })
  },
  success(state, { desc, busy, extraMsgs }) {
    return update(state, {
      ...
      busy: { $set: busy - 1 },
      show: {
        $set: extraMsgs.some(m => m.kind === 'warning' || m.kind === 'error'),
      },
    })
  },
  error(state, { desc, busy, extraMsgs }) {
    return update(state, {
      messages: {
        $push: [...extraMsgs, { kind: 'error', text: `${desc} failed` }],
      },
      busy: { $set: busy - 1 },
      show: { $set: true },
    })
