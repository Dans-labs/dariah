...
const templates = {
  ...
  detail: {
    assessment({ l, v, e, s, f }) {
      const statusClass =
        e('evidence') || e('score') ? 'incomplete' : 'complete'
      return (
        <div className={`criteria entryRead ${statusClass}`}>
          <div className={'criteria entry'}>
            ...
            {e('evidence') ? (
              <div className={'xSlim'}>
                <Tooltip tip={'No evidence yet'} at={'top'}>
                  <span className={'slim fa fa-file-o tError'} />
                </Tooltip>
              </div>
            ) : (
              <ExpandHead
      ...
      return (
        <div className={`criteria entryEdit ${statusClass}`}>
          <div className={'criteria entry'}>
            ...
            <div className={'slim'}>
              {fe('score', { suppressTyping: true })}
            </div>
          </div>
          <p>
            <b>{l('evidence')}</b>
          </p>
          <Tooltip tip={'Give evidence'} at={'top'}>
            {fe('evidence')}
          </Tooltip>
        </div>
      )
